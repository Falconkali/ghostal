...
export default function Topbar({ onMobileMenuToggle, sidebarCollapsed }: TopbarProps) {
  ...
  return (
    <header
      className={cn(
        "fixed top-0 right-0 z-30 flex h-16 items-center gap-4 border-b border-white/5 px-4 md:px-5 glass transition-all duration-300",
        sidebarCollapsed ? "md:left-16" : "md:left-[200px]",
        "left-0"
      )}
    >
      {/* Mobile menu toggle */}
